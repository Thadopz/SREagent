...
import (
	"SREagent/internal/ai/agent/knowledge_index_pipeline"
	loader2 "SREagent/internal/ai/loader"
...
func main() {
	...
	err = filepath.WalkDir("./docs", func(path string, d fs.DirEntry, err error) error {
		...
		source := strings.ReplaceAll(fmt.Sprint(docs[0].MetaData["_source"]), `\`, `\\`)
		source = strings.ReplaceAll(source, `"`, `\"`)
		expr := fmt.Sprintf(`metadata["_source"] == "%s"`, source)
		queryResult, err := cli.Query(ctx, common.MilvusCollection(ctx), []string{}, expr, []string{"id"})
		if err != nil {
			return err
			...
			if len(idsToDelete) > 0 {
				...
				err = cli.Delete(ctx, common.MilvusCollection(ctx), "", deleteExpr)
				if err != nil {
